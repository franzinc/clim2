...
When the sheet's transformation is changed, \cl{note-sheet-region-changed} is
...
When the sheet's region is changed, \cl{note-sheet-region-region} is called
...
\issue {SWM} {The above three functions should really be \cl{map-over-...}
...
\issue {SWM} {Do we want to advertise \cl{sheet-event-queue}?  It is analogous
...
\Defclass {pointer-button-click-and-hold-event}
...
are subclasses of \cl{pointer-button-event}.  Ports are not required to generate
these events.
...
\issue {SWM} {RSL claims that this is old and obsolete.  What is the proposed
replacement for it?}
