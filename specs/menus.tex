...
\item \cl{:items}---specifies a sequence of menu items for a sub-menu used if
...
\cl{:separator} indicates that the item serves as a separator between groups of
...
\arg{default-style} is a text style that defines how the menu items are
presented.
...
\Defgeneric {menu-choose-from-drawer} {menu type drawer
...
\arg{stream} and \arg{type}.  It can use \arg{type} for its own purposes, such
as using it as the type argument in a call to \cl{present}.
...
\arg{type} is a presentation type specifier for each of the mouse-sensitive
...
\arg{type}, draws the contents of the menu.  It has dynamic extent.
...
\arg{n-rows}, \arg{n-columns}, \arg{inter-column-spacing},
\arg{inter-row-spacing}, \arg{cell-align-x}, and \arg{cell-align-y} are as for
\cl{menu-choose}

\Defun {print-menu-item} {menu-item \optional stream}
