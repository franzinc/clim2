...
The output record class that represents presentations.  \cl{present} normally
...
\arg{env} is a macro-expansion environment, as in \cl{macroexpand}.
...
\Defpresmeth {presentation-refined-position-test} {}
...
\cl{with-input-context}, and the last element is the least recent call to
...
defaults to \cl{*query-io*}.  \cl{accept} returns two values, the object
...
The class representing all textual views, a subclass of \cl{view}.  Presentation
methods that apply to a textual view must only do textual input and output (such
as \cl{read-char} and \cl{write-string}).

\Defclass {textual-menu-view}

The class that represents the default view that is used inside \cl{menu-choose}
for frame managers that are not using a gadget-oriented look and feel.  It is a
subclass of \cl{textual-view}.
...
documentation.  It is a subclass of \cl{textual-view}.
...
\\
\arg{(translator the-presentation context-type tag)}
\\
...
context type that in which the translator applies, and \arg{tag} is a tag used
internally by CLIM.  \arg{translator}, \arg{the-presentation}, and
\arg{context-type} can be passed to such functions as
\cl{call-presentation-translator} and \cl{document-presentation-translator}.
