...
\Defun {extended-output-stream-p} {object}
...
Returns (or sets) the visibility of the \term{cursor} \arg{cursor}.  The
visibility can be either \cl{:on} (meaning that the cursor will be visible at
its current position), \cl{:off} (meaning that the cursor is active, but not
visible at its current position), or \cl{nil} (meaning that the cursor is to
be deactivated).

\Defgeneric {display-cursor} {cursor state}

This is responsible for drawing or erasing the \term{cursor} \arg{cursor}.
If \arg{state} is \cl{:draw}, the cursor will be drawn.  Otherwise, if the
\arg{state} is \cl{:erase}, the cursor will be erased.
...
as a rational number.  \arg{text-style} defaults to the current text style for
the stream.
...
\Defmacro {with-room-for-graphics} {(\optional stream
                                     \key (move-cursor \cl{t}) height record-type)
                                    \body body}

Binds the dynamic environment to establish a local Cartesian coordinate system
for doing graphics output onto the \term{extended output stream} designated by
\arg{stream}.  The origin $(0,0)$ of the local coordinate system is placed at
the current cursor position, and is in the lower left corner of the area
created.  If the boolean \arg{move-cursor} is \term{true} (the default), then
...
\Defgeneric {beep} {\optional sheet}
