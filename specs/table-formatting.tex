...
\Defmacro {formatting-row} {(\optional stream \key record-type)
                            \body body} 
...
\Defmacro {formatting-column} {(\optional stream \key record-type)
                               \body body} 
...
\arg{max-width}, \arg{max-height}, \arg{initial-spacing}, and \arg{move-cursor}
are as for \cl{formatting-item-list}.
...
\Defun {item-list-output-record-p} {object}
...
the desired number of rows and columns, and maximum width and height attributes
of the item list.

\Defclass {standard-item-list-output-record}

The output record that represents item list output.  Its children will be a
sequence of cells, with presentations possibly intervening.  This is a subclass
of \cl{item-list-output-record}.
