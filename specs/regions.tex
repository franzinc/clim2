...
example) canonicalizes it to \cl{+nowhere+}.
...
\cl{make-line} with two coincident points), for example) canonicalizes it to
\cl{+nowhere+}.
...
\cl{make-rectangle} with two coincident points), for example) canonicalizes it
to \cl{+nowhere+}.
...
The type that represents a coordinate.  This must be a subtype of \cl{real}, but
CLIM implementations may use a more specific subtype (such as \cl{single-float})
...
\cl{region-union} will return either a simple region or a member of the class
\cl{standard-region-union}.
...
\cl{region-difference} will return either a simple region or a member of the
class \cl{standard-region-difference}.
...
\Issue {SWM, York} {There is a proposal to remove the \cl{polygon}, \cl{polyline},
\cl{line}, \cl{ellipse}, and {elliptical-arc} classes, since they are only of
