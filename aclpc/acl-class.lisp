...
(progn
   ...
   (cg::defbvar *window-proc-result* (cg::callocate :long))
...
(defun clim-window-proc (x)
  ...
  (let ((d (cg::pccstructure-data-pointer x)))
    (setf (cg::lhandle-value *hwnd*) (cg::long-ref d 0))
  ...
  (let ((result (cg:ccallocate (:void *))))
    (setf (pc::cpointer-value result)
          (cg::word-vector-long-ref *window-proc-result* 0))
...
(defun init-clim-win-proc (wproc-address arg-struct)
  (setf (pc::cpointer-value wproc-address)
        (cg::wprocinit2 (ff:register-function 'clim-wind-proc)))
  (setf *clim-wproc-arg-struct* arg-struct))

#+acl86win32
(defun init-clim-ctrl-proc (wproc-address arg-struct)
  (setf (pc::cpointer-value wproc-address)
        (cg::wprocinit3 (ff:register-function 'clim-ctrl-proc)))
...
#+aclpc
(defvar pc::cbn_closeup 8)

(#+aclpc ct:defun-callback #+acl86win32 ff:defun-c-callable clim-wind-proc
  ...
  #+acl86win32 (progn (setf (cg::lhandle-value *hwnd*) window) (setq window *hwnd*))
  ...
  (let ((result (ct::ccallocate (win:void *)))
	(*level* (1+ *level*)))
    (when (> *level* 40)
      (break "too deep!"))
	...
	(note-pointer-motion *acl-port* sheet mx my)))
     ...
     ((eq msg win:wm_setcursor)
	  (unless (maybe-set-cursor window)
		#+acl86win32
		(win::defwindowproc window msg wparam lparam *win-result*)
		#+aclpc
	...
	(let* ((udrect (ct::ccallocate win:rect))
	 ...
	 (let ((sheet (mirror->sheet *acl-port* hwnd)))
	   (when sheet
		 #+acl86win32 ;+++ fixme
	     (setf (cg::word-vector-long-ref *win-result* 0)
	       (pc::lhandle-value
			 (adjust-gadget-colors sheet hdc)))))))
         ...
         (if (and (= lparam 0)   ; menu item
             ...
             (progn 
               ...
               (if (typep gadget 'silica::mswin-combo-box-pane)
                   (let ((sheet (mirror->sheet *acl-port* window)))
                     (when (and sheet (= whiword pc::cbn_closeup))
                       (with-slots (event-queue) *acl-port*
                         (queue-put
                           event-queue
                           (multiple-value-bind
               ...
               (when (or (not (typep gadget 'silica::hlist-pane))
                 ...
                 (when (typep gadget 'silica::hlist-pane) (win::setfocus window))
                 (with-slots (event-queue) *acl-port*
		 ...
		 (multiple-value-bind (action amount)
							  (cond ((eql type win::sb_lineup)
									...
									((eql type win::sb_bottom)
									 (win::setScrollPos window flag 100 1)
       ...
       (let* ((code wparam)
         ...
         (let* ((capstate (win::getKeyState win:vk_capital))
           ...
           (setf (modstate-meta *modstate*)
                 (or (minusp metastate) (not (zerop (ash controlstate -15)))))
...
#+acl86win32
    (setf (pc::cpointer-value result)
          (cg::word-vector-long-ref *win-result* 0))
#+acl86win32
    (cg::wprocsetter2 result)))
)
...
(defun acl-clim::register-window-class ()
  ...
  (unless *wndclass-registered*
    (init-clim-win-proc clim-window-proc-address #.(cg::make-cstructure 0 16))
    (let ((class (cg:ccallocate win:wndclass)))
      (ct::csets win:wndclass class
                 win:style win:CS_DBLCLKS
      ...
      (setq err1 (win::getLastError))
      (unless (setq reg (win::registerclass class))
  ...
  (let* ((overlap (if ovl
    ...
    (let ((window (#+acl86win32 win::createWindowEx #+acl86win32 0
                  ...
                  *win-name*
                  ...
                  (ct::null-handle win:hmenu)
                  pc::*hinst*
                  #+acl86win32 (setq *win-x* (symbol-name (gensym)))
				  #+aclpc *win-arg*)))
	  #+acl86win32
      (if (cg::null-handle-p window window)
...
(defun await-response (waitp)
  (let (#+aclpc (msg (ct::ccallocate win:msg)))
    (if waitp
      (progn 
        (win::getMessage msg (ct::null-handle win:hwnd) 0 0 #+acl86win32 res)
        (win::TranslateMessage msg  #+acl86win32 :static)
        (win::dispatchMessage msg)
	msg)
      (when (prog1
              (win::peekMessage msg (ct::null-handle win:hwnd) 0 0 win:PM_REMOVE  #+acl86win32 res)
               #+acl86win32
			   (not (and (zerop (cg::hiword res)) (zerop (cg::loword res)))))
        (win::TranslateMessage msg  #+acl86win32 :static)
        (win::dispatchMessage msg)))))
