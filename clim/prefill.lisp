...
;; $fiHeader: prefill.lisp,v 1.4 91/03/26 12:48:29 cer Exp $
...
(PREFILL-DISPATCH-CACHES
  ...
  (STREAM-MERGED-TEXT-STYLE-VALID
    (#+Cloe-Runtime CLOE-WINDOW-STREAM #+Genera SHEET-WINDOW-STREAM #+Minima CLX-WINDOW))
  ((SETF STREAM-MERGED-TEXT-STYLE-VALID)
   (T #+Cloe-Runtime CLOE-WINDOW-STREAM #+Genera SHEET-WINDOW-STREAM #+Minima CLX-WINDOW))
...
(PREFILL-DISPATCH-CACHES
  (ACCEPT-1 (STANDARD-INPUT-EDITING-STREAM T)
...
(PREFILL-DISPATCH-CACHES
  (EVENT-WINDOW (POINTER-BUTTON-PRESS-EVENT POINTER-BUTTON-RELEASE-EVENT))
...
(PREFILL-DISPATCH-CACHES
  ((SETF TEXT-STYLE-MAPPING) #+Cloe-Runtime (T CLOE-DEVICE T T) 
			     #+Genera (T SHEET-DEVICE T T)
			     #+Minima (T CLX-DISPLAY-DEVICE T T)
			     (T POSTSCRIPT-DEVICE T T))
  (DISPLAY-DEVICE-NAME #+Cloe-Runtime (CLOE-DEVICE)
		       #+Genera (SHEET-DEVICE)
		       #+Minima (CLX-DISPLAY-DEVICE)
		       (POSTSCRIPT-DEVICE)) 
  (STANDARDIZE-TEXT-STYLE #+Cloe-Runtime (CLOE-DEVICE T T) 
			  #+Genera (SHEET-DEVICE T T)
			  #+Minima (CLX-DISPLAY-DEVICE T T)
			  (POSTSCRIPT-DEVICE T T)) 
  (TEXT-STYLE-MAPPING #+Cloe-Runtime (CLOE-DEVICE T TEXT-STYLE)
		      #+Genera (SHEET-DEVICE T TEXT-STYLE)
		      #+Minima (CLX-DISPLAY-DEVICE T TEXT-STYLE)
		      #+Genera (SHEET-DEVICE T CONS)) 
  (TEXT-STYLE-MAPPING* #+Cloe-Runtime (CLOE-DEVICE T T)
		       #+Genera (SHEET-DEVICE T T)
		       #+Minima (CLX-DISPLAY-DEVICE T T)))
