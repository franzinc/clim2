...
;; $fiHeader: genera-activities.lisp,v 1.4 91/03/26 12:47:58 cer Exp $
...
(scl:defmethod (sys:select-activity clim-activity)
  ...
  (cli::find-frame-for-activity console superior
    #'(lambda (clim-sheet)
	(when (typep clim-sheet 'clim-sheet)
	  (let* ((window (clim-window-for-host-window clim-sheet :error-if-no-match nil))
...
(scl:defmethod (cli::activity-frame-acceptable-p clim-activity) (clim-sheet)
  (and (typep clim-sheet 'clim-sheet)
       (let* ((window (clim-window-for-host-window clim-sheet :error-if-no-match nil))
	      (frame (and window (window-stream-to-frame window))))
	 (and frame
