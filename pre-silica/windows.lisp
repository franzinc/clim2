...
;; $fiHeader: windows.lisp,v 1.3 92/05/18 14:04:11 cer Exp Locker: cer $
...
(defun polygon (dc list-of-points &optional (x-offset 0) (y-offset 0))
  (declare (fixnum dc))
  (let* ((len (length list-of-points))
	 (alen (+ (* 2 len) 4)))
    ...
    (cond ((< alen 255)
	   (with-fast-command (command *VM-BCOM-POLYGON* alen)
		 ...
		 (store-16bit command ind (round (+ (pop list) x-offset)))
		 (incf ind 2)
		 (store-16bit command ind (round (+ (pop list) y-offset)))
		 (incf ind 2)))))
	  (t
	   ...
	   (let ((list list-of-points)
	     ...
	     (loop
	       (when (null list) (return))
	       (store-16bit *polygon* ind (round (+ (pop list) x-offset)))
	       (incf ind 2)
	       (store-16bit *polygon* ind (round (+ (pop list) y-offset)))
	       (incf ind 2)))
	   (send-command-string *VM-BCOM-POLYGON* *polygon*)))))

(defun polyline (dc list-of-points &optional closed (x-offset 0) (y-offset 0))
  (declare (fixnum dc x-offset y-offset))
  (let* ((len (+ (length list-of-points) (if closed 2 0)))
	 (alen (+ (* 2 len) 4)))
    (declare (fixnum len alen))
    (when (oddp len) (error "Polygons must have even number of points"))
    (cond ((< alen 255)
	   (with-fast-command (command *VM-BCOM-POLYLINE* alen)
		 ...
		 (incf ind 2))
	       (when closed
		 (store-16bit command ind (round (+ (first list-of-points) x-offset)))
		 (incf ind 2)
		 (store-16bit command ind (round (+ (second list-of-points) y-offset)))
		 (incf ind 2)))))
	  (t
	   (when (< (array-total-size *polygon*) alen)
	     (setf *polygon* (make-array alen :element-type 'string-char :fill-pointer 0)))
	   ...
	   (let ((list list-of-points)
	     ...
	     (loop
	       ...
	       (incf ind 2))
	     (when closed
	       (store-16bit *polygon* ind (round (+ (first list-of-points) x-offset)))
	       (incf ind 2)
	       (store-16bit *polygon* ind (round (+ (second list-of-points) y-offset)))
	       (incf ind 2)))
