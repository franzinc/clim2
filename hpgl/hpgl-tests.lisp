...
;;; We should feel ashamed: This file is postscript-tests with a
;;; s/postscript/hpgl/

;; $fiHeader: hpgl-tests.lisp,v 1.4 92/12/16 16:50:10 cer Exp $

(in-package :hpgl-clim)

;;; This file contains a whole bunch of useful graphics and text
;;; output tests that can be used for generating hpgl also.

(define-application-frame clim-hpgl-tests ()
    ()
  (:panes
...
(define-clim-hpgl-tests-command (com-exit-clim-hpgl-tests :name t) 
    ()
...
(defmacro sending-hpgl-to-editor-buffer ((stream-var buffer-name &rest options)
					       &body body)
  #+Allegro
  (let ((editor-stream-var '#:editor-stream))
    `(lep:with-output-to-temp-buffer (,editor-stream-var ,buffer-name)
       (with-output-to-hpgl-stream (,stream-var ,editor-stream-var ,@options)
	 ,@body)))
  #+Genera
...
(defparameter *hpgl-test-buffer-name* "hpgl-test")

(defmacro with-hpgl-test-output-to-buffer ((stream-var &rest options)
						 &body body)
  `(sending-hpgl-to-editor-buffer (,stream-var *hpgl-test-buffer-name*
					 ,@options)
					,@body))

#+Genera (zwei:defindentation (define-hpgl-test 0 5 1 5 2 1))

#+Allegro
(defvar *allegro-printer* "lw2")

(defvar *hpgl-test-default-output* :view)

(defvar *hpgl-tests* nil)

(defmacro define-hpgl-test
  ...
  (let* ((command-name-string (command-name-from-symbol root-name))
	 ...
	 (additional-key-args '((output '(member :view :file :buffer #+allegro :printer)
					:default *hpgl-test-default-output*)))
	 (hpgl-test-name (clim-utils:fintern "~A~A" 'pcom- (string root-name)))
	 (args arguments)
	 (command-body-form
    ...
    `(#+Genera sys:multiple-definition #+Genera ,root-name #+Genera define-hpgl-test
      #-Genera progn
      (defun ,command-body-function-name ,(clim-internals::deduce-body-arglist arguments)
	,@body)
      (pushnew ',hpgl-test-name *hpgl-tests*)
      (define-command (,hpgl-test-name
		       :command-table clim-hpgl-tests
	...
	(ecase output
	  ...
	  (:file
	    (with-open-file
	        (file-stream "hpgl.output" :direction :output :if-exists :supersede)
	      (with-output-to-hpgl-stream (*standard-output* file-stream)
		,command-body-form)))
	  (:printer 
	    #+Allegro
	    (with-open-stream 
		...
		(excl:run-shell-command (format nil
						"/usr/tech/cer/3rd/hpgl2ps-v2/hpgl2ps | lpr -P~A" *allegro-printer*)
					:input :stream :wait
					nil))
	      (with-output-to-hpgl-stream (*standard-output* printer-stream-var)
		,command-body-form))
	    #-Allegro
	    (error "Sorry, don't know how to send to printer"))
	  (:buffer
	    (with-hpgl-test-output-to-buffer
...
;	   (funcall doit w)))
;	((null file)
;	 (with-output-to-hpgl-stream (s *standard-output*)
;	   (funcall doit s)))
;	(t
;	 (with-open-file (file file :direction :output)
;	   (with-output-to-hpgl-stream (s file)
;	     (funcall doit s))))))
...
(define-hpgl-test (pattern-test "Draws a heart patterned rectangle"
				      clim-user::graphics)
			()
...
(define-hpgl-test (draw-line-test "Draws a line" )
			()
...
(define-hpgl-test (test-ellipse "Draw an ellipse")
			()
...
(define-hpgl-test (test-graphics "Test graphics primitives" clim-user::graphics)
			()
  (let ((stream *standard-output*))
...
(define-hpgl-test (test-alignment "See if text falls on rules")
			()
  (let ((stream *standard-output*))
...
(define-hpgl-test (write-multiple-strings
...
(define-hpgl-test (test-character-positioning "Draw a big character"
						    clim-user::graphics)
			()
  (let ((stream *standard-output*)
	(x 100)
...
(define-hpgl-test (test-text-vertical-alignment
...
(define-hpgl-test (test-table "Draw a table" clim-user::formatted-output)
			()
  (let ((stream *standard-output*))
    (fresh-line stream)
...
(define-hpgl-test (test-text-size "Is text 10 units high?")
			()
  (let ((stream *standard-output*))
    (draw-line* stream 100 100 300 100)
...
(define-hpgl-test (test-text-sizes "Output text in several sizes"
					 clim-user::graphics)
			()
  (let ((stream *standard-output*))
    (flet ((out (size)
...
(define-hpgl-test (test-text-baselines
...
(define-hpgl-test (test-text-alignment "Text drawing and alignment"
					     clim-user::graphics)
...
(define-hpgl-test (test-transformations "")
			()
  (let ((stream *standard-output*)
...
(define-hpgl-test (test-record-and-replay
...
(define-hpgl-test (test-with-room-for-graphics "" clim-user::formatted-output)
			()
  (let ((stream *standard-output*))
    (fresh-line stream)
...
(define-hpgl-test (show-dash-patterns
...
(define-hpgl-test (show-dash-patterns-some-more
...
(define-hpgl-test (partial-circle "circle with start angle and end angle in degrees")
...
(defun run-hpgl-tests (&key (output :view))
  (clim-user::exercise-frame 'clim-hpgl-tests
