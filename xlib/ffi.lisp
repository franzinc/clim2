     ...
     (eval-when (compile eval load)
       ,(let ((c-name (second (assoc :name options)))
	      (return-type (or (second (assoc :return-type options))
			       'void))
	      (strings-convert (assoc :strings-convert options))
	      (call-direct (assoc :call-direct options))
	      extras)
	  (setq extras
	    (nconc extras
		   (if call-direct
		       (list :call-direct (second call-direct))
		     (list :call-direct t))))
	  (when strings-convert
	    (setq extras (nconc extras (list :strings-convert (second strings-convert)))))
